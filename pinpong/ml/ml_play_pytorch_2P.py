...
class MLPlay:
    ...
    RETURN_CENTER_ZONE = 0.35
    FORCE_CHASE_DIST = 10.0

    MODEL_PATH = os.getenv("PPO_MODEL_2P", "ppo_pong_2P.pt")
    ...
    def __init__(self, ai_name: str, *args, **kwargs):
        ...
        if not os.path.isfile(self.MODEL_PATH):
            raise FileNotFoundError(f"[MLPlay-2P] 找不到模型檔：{self.MODEL_PATH}")
        ...
        print(f"[MLPlay-2P] Loading PPO model from: {self.MODEL_PATH}")
        print("[MLPlay-2P] Model loaded successfully.")
        print("2P init OK")
    ...
    def _make_obs_2p(self, scene_info: dict) -> np.ndarray:
        bx, by = scene_info["ball"]
        vx, vy = scene_info.get("ball_speed", [0, 0])

        # 翻轉 y / vy（讓 2P 也像 1P 一樣理解「球往自己來」）
        by2 = self.HEIGHT - float(by)
        vy2 = -float(vy)

        # 在翻轉視角下：自己當 p1，對手當 p2
        p1x = float(scene_info["platform_2P"][0])
        p2x = float(scene_info["platform_1P"][0])
        ...
        by_n = (by2 / self.HEIGHT) * 2.0 - 1.0
        vx_n = float(np.clip(float(vx) / self.MAX_V, -1.0, 1.0))
        vy_n = float(np.clip(vy2 / self.MAX_V, -1.0, 1.0))
    ...
    def _postprocess_cmd(self, scene_info: dict, cmd: str) -> str:
        px = float(scene_info["platform_2P"][0])
        bx, by = scene_info["ball"]
    ...
    def update(self, scene_info: dict, keyboard_info=None) -> str:
        ...
        obs = self._make_obs_2p(scene_info)
